...
public class User implements UserDetails {
    ...
    @Size(min = MIN_LENGTH_OF_NAME, max = MAX_LENGTH_OF_NAME, message = "Name must be between 3 and 30 characters long.")
	private String name;

	@Column(name = "SURNAME", nullable = false, length = MAX_LENGTH_OF_FIELD_IN_DB)
    @Pattern(regexp="^[а-яА-ЯІіЇїЄєa-zA-Z0-9]+$",
            message="Surname must be alpha numeric with no spaces")
    @Size(min = MIN_LENGTH_OF_SURNAME, max = MAX_LENGTH_OF_SURNAME, message = "Surname must be between 3 and 30 characters long.")
    ...
    @Size(min = MIN_LENGTH_OF_NICKNAME, max = MAX_LENGTH_OF_NICKNAME, message = "Nickname must be between 3 and 30 characters long.")
